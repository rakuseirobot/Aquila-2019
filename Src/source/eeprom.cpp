...
void erom::WriteSingle(uint16_t eeaddress,uint8_t data ) //0~31999
{
	...
	HAL_I2C_Master_Transmit(&lcd_i2c, EEPROM_ADDR<<1,tx,3, 100);
    HAL_Delay(20);
    return;
}
...
{   
    ...
    HAL_I2C_Master_Transmit(&lcd_i2c, EEPROM_ADDR<<1,tx,66,100);
    HAL_Delay(20);
    return;
}
...
uint8_t erom::ReadSingle(uint16_t eeaddress) 
{
    ...
    HAL_I2C_Mem_Read(&lcd_i2c,EEPROM_ADDR,eeaddress,I2C_MEMADD_SIZE_16BIT,rdata,1,100);
    return rdata[0];
}
...
void erom::ReadMulti(uint16_t eeaddress,uint8_t *d,uint16_t val) //どこ指定してもいい!
{
    ...
    HAL_I2C_Mem_Read(&lcd_i2c,EEPROM_ADDR,eeaddress,I2C_MEMADD_SIZE_16BIT,d,val,100);
    return;
}
